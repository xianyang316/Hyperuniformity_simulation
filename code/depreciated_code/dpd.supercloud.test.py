...
dpd.pair_coeff.set('A', 'B', A=25.0, gamma = 1.0);
dpd.pair_coeff.set('B', 'B', A=100.0, gamma = 1.0);
...
hoomd.analyze.log(filename="log-dpd.hairy.log",
                  ...
                  period=100,
                  overwrite=True)
hoomd.dump.gsd("dpd.hairy.gsd", period=1e5, group=hoomd.group.all(), overwrite=True, dynamic=["momentum"])
