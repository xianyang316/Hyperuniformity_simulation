...
sigma_BB = d_polymer
print(sigma_AA, sigma_AB, sigma_BB)
...
# fene.bond_coeff.set("bondAB", k=30.0, r0=2.0*((d_center+d_polymer-2)/2), sigma=(d_center+d_polymer-2)/2, epsilon=1.0)
fene.bond_coeff.set("bondAB", k=30.0, r0=1.5*d_center, sigma=1.0, epsilon=0.0)
# # fene.bond_coeff.set("bondBB", k=30.0, r0=5*d_polymer, sigma=2.5*d_polymer, epsilon=0.1)
fene.bond_coeff.set("bondBB", k=30.0, r0=1.5*(d_polymer), sigma=1.0, epsilon=0.0)
...
hoomd.analyze.log(filename="log-dpd.single.log",
                  ...
                  period=100,
                  overwrite=True)
# hoomd.dump.gsd("slj.single.fene.gsd", period=1e4, group=hoomd.group.all(), overwrite=True, dynamic=["momentum"])
